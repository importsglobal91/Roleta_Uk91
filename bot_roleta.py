...
class BotMultiRoleta:
    def __init__(self):
        ...
        # 14 mesas 32Red reais (expandidas)
        self.roletas_nomes = [
            "Roleta 32Vermelha", "Lightning Roulette", "Quantum Roulette",
            "Auto Roulette", "VIP Roulette", "Dragonara Roulette",
            "French Roulette", "American Roulette", "Mega Roulette",
            "Speed Roulette", "Immersive Roulette", "Slingshot Auto",
            "Double Ball Roulette", "Crazy Time Roulette"
        ]
    ...
    async def gerar_numero_real(self, nome_roleta: str) -> int:
        """Futuro: Selenium scraper 32Red (integrado do file:14)"""
    ...
    async def processar_numero(self, nome_roleta: str, numero: int):
        ...
        if "9 " in sinal or "REPETIÇÃO" in sinal or roleta.contagem >= 10:
            roleta.greens += 1
            msg_sinal = f"🟢 **SINAL {nome_roleta}!**\n📊 Nº: {numero}\n{sinal}\n🟢 Total: {roleta.greens}"
            print(f"🟢 SINAL: {sinal}")
            if self.context and self.chat_id:
                await self.context.bot.send_message(chat_id=self.chat_id, text=msg_sinal, parse_mode="Markdown")
    ...
    async def enviar_sinal(self, mensagem: str):
        try:
            await self.context.bot.send_message(chat_id=self.chat_id, text=mensagem, parse_mode="Markdown")
...
async def iniciar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sucesso:
        ...
        await update.message.reply_text("✅ **BOT LIGADO!** 14 mesas ativas\nUse /status", parse_mode="Markdown")
...
async def roletas(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = "🎯 **14 Mesas 32Red**\n" + "\n".join(f"{i+1}. {nome}" for i, nome in enumerate(botauto.roletas_nomes))
    await update.message.reply_text(msg, parse_mode="Markdown")

async def parar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    botauto.parar()
    total = sum(r.greens for r in botauto.roletas.values())
    await update.message.reply_text(f"⏹️ Parado | 🟢 Total sinais: {total}", parse_mode="Markdown")
